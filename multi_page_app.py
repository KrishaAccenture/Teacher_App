...
# Function to generate lesson plan content using OpenAI
...
def generate_ppt_slides(api_key, lesson_plan):
    openai.api_key = api_key
...
def generate_activity_sheets(api_key, lesson_plan, parsed_ppt_content):
    openai.api_key = api_key
...
def add_to_history(user_input, lesson_plan, ppt_content, activity_sheet_content):
    entry = {
...
def display_sidebar_history():
    with st.sidebar:
        ...
        for index, entry in enumerate(st.session_state.history):
            if st.button(f"Session {index + 1}", key=f"history_{index}"):
                show_history_entry_details(index)

# Function to show details of a history entry
def show_history_entry_details(index):
    entry = st.session_state.history[index]
    st.write(f"Details for Session {index + 1}")
    st.write(f"Subject: {entry['user_input']['subject']}, Lesson Topic: {entry['user_input']['lesson_topic']}")
    st.download_button("Download Lesson Plan", entry['lesson_plan'])
...
def main():
    ...
    if st.button('Generate Materials'):
        with st.spinner('Generating materials...'):
            ...
            ppt_content = generate_ppt_slides(user_input, lesson_plan)
            activity_sheet_content = generate_activity_sheets(user_input, lesson_plan, ppt_content)
            ...
            st.download_button("Download Lesson Plan", lesson_plan)
            st.download_button("Download PowerPoint", ppt_content)
            st.download_button("Download Activity Sheets", activity_sheet_content)
